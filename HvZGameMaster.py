...
class menu(object):
	...
	def mission(self):
		...
		try:
			...
			self.messenger.sendZombies("Zombies, new mission!\nCheck your email for details.")
		except Exception, e:
			raise
												...
												if playerManager.pDict[candidate].number != self.mail[0]:
													if not playerManager.pDict[candidate].zombie:
														if candidate not in set(helpers):
															...
															count += 1
												watchdog += 1
											msg = "\nSomeone needs help at " + processedMessage[1]
											self.messenger.sendPIDs(helpers, msg)
											self.messenger.sendSingle(self.mail[0], "\nRequest for help received and forwarded")

											#sets the cooldown
											playerManager.pDict[key].activeCooldown = time.time()
										else:
											self.messenger.sendSingle(self.mail[0], "\nYour ability to call for help is on cooldown.\nCooldown lasts for one hour after you use it. ")
									else:
										self.messenger.sendSingle(self.mail[0],"\n")
								...
								if dif > 3599:
									...
									if processedMessage[0] != self.mail[1]:
										if playerManager.pDict[key].PZero or playerManager.pDict[key].zombie:
											if	re.match('tcb|kennedy center|east|science center|lowry|lowry hall|library|kennedy|tunheim classroom building|beadle',processedMessage[1], flags=re.I)!=None:
												self.messenger.sendZombies("\nSiege " + processedMessage[1])
												playerManager.pDict[key].activeCooldown= time.time()

											elif re.match('mission*|tcb.*|kennedy center.*|science center.*|lowry.*|east.*|lowry hall|library|kennedy|tunheim classroom building|beadle', processedMessage[1], flags=re.I)!=None:
												self.messenger.sendZombies("\nSiege " + processedMessage[1][-5:] + "at" + processedMessage[1][:-5]+".")
												playerManager.pDict[key].activeCooldown= time.time()
											else:
												self.messenger.sendSingle(self.mail[0],"Malformed siege command")
										else:
											self.messenger.sendSingle(self.mail[0], "Sieges can only be initiated by PZ or zombies.")
									else:
...
class player(object):
	...
	#returns a tupple of the name, phone number(formatted as an email address) and email address of the playes.
	def getAttribs(self):
...
class messaging(object):
	...
	def __init__(self,fromaddr,playerManager):
		...
		print "Atempting to log in to %s (username: %s)" % (self.fromaddr, args.username)
		try:
	...
	def sendPIDs(self, pIDlist, msg):
		for pIDs in pIDlist:
			#print pIDs
			self.server.sendmail(self.fromaddr, playerManager.pDict[pIDs].number, msg)
			time.sleep(1)
...
#############################################################################
##a couple of utility functions that are up here for cleanliness and	   ##
##that really only main (and truthfully not even main) has to care about.  ##
#############################################################################
